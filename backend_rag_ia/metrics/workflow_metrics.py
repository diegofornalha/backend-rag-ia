...
class WorkflowMetrics:
    ...
    def record_state_change(self, embate_id: str, 
    ...
    def record_operation(self, embate_id: str, operation: str) -> None:
        ...
        logger.info(
            f"Operação {operation} registrada para embate {embate_id} "
            f"em {timestamp}"
        )
    ...
    def _calculate_avg_cycle_time(self) -> Optional[float]:
        ...
        cycle_times = [
            self.get_cycle_time(embate_id)
            for embate_id in self.state_changes.keys()
        ]
        ...
        total_seconds = sum(
            ct.total_seconds() for ct in valid_times
        )
