...
class GlobalEmbatesCounter:
    ...
    def increment(self, embate_id: str) -> None:
        ...
        logger.info(f"Chamada registrada - Embate: {embate_id}, "
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            'most_active_embate': max(self._calls_by_embate.items(), 
                                    key=lambda x: x[1], 
                                    default=(None, 0)),
            'last_call': max(self._last_call_time.values(), 
                           default=None) if self._last_call_time else None
        } 