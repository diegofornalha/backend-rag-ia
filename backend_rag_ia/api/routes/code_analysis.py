...
async def analyze_code(
    ...
    try:
        ...
        result = await llm_manager.process_task(
            task="code_analysis",
        ...
        if not analysis_data:
            raise HTTPException(
                status_code=500,
                detail="Nenhuma análise foi gerada"
            )
        ...
        response = {
            "analysis": {
                ...
                "security_issues": analysis_data[1].get("analysis", {}) if len(analysis_data) > 1 else {}
            },
...
async def improve_code(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erro ao gerar melhorias: {str(e)}"
        ) 