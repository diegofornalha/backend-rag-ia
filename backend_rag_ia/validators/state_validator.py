...
class StateValidator:
    ...
    TRANSITION_REQUIREMENTS = {
        'em_andamento': {
            'min_arguments': 1,
            'required_types': {'analise'}
        },
        'fechado': {
            'min_arguments': 2,
            'required_types': {'analise', 'solucao'}
        }
    }
    ...
    def validate_state_change(
        embate: Dict,
        new_state: str,
        allow_requirements_override: bool = False
    ) -> List[str]:
        """
        ...
        if not allow_requirements_override:
            ...
            requirement_errors = StateValidator.validate_requirements(
                new_state,
                arguments
            )
    ...
    def get_cycle_time(embate: Dict) -> Optional[float]:
        ...
        try:
            ...
            start_date = datetime.fromisoformat(
                history[0]['data'].replace('Z', '+00:00')
            )
            ...
            if current_state == 'fechado':
                ...
                end_date = datetime.fromisoformat(
                    last_change['data'].replace('Z', '+00:00')
                )
            else:
