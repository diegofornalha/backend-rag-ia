...
class ReferenceValidator:
    ...
    def get_reverse_references(self, embate_id: str) -> List[Dict]:
        ...
        try:
            ...
            logger.error(
                f"Erro ao buscar referências reversas para {embate_id}: {str(e)}"
            )
            return []
        ...
        try:
            ...
            for ref_id in references:
                ...
                if embate.get('id') in self.get_references(ref_embate):
                    errors.append(
                        f"Referência cíclica detectada entre "
                        f"{embate.get('id')} e {ref_id}"
                    )
    ...
    def check_orphaned_references(self) -> List[Dict]:
        ...
        try:
            ...
            return [{
                'tipo': 'erro',
                'mensagem': f"Erro ao verificar referências órfãs: {str(e)}"
            }]
    ...
    def get_reference_graph(self, embate_id: str) -> Dict:
        ...
        try:
            ...
            logger.error(
                f"Erro ao gerar grafo de referências para {embate_id}: {str(e)}"
            )
            return {'nodes': [], 'edges': []} 