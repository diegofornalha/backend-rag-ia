...
class SchemaValidator:
    ...
    BASE_SCHEMA = {
        ...
        "properties": {
            ...
            "tipo": {"type": "string", "enum": ["feature", "bug", "processo", "tech_debt"]},
            "status": {
                "type": "string",
    ...
    FEATURE_SCHEMA = {
        "type": "object",
        "required": ["contexto"],
        "properties": {"contexto": {"type": "string", "minLength": 1}},
    }
    ...
    def sanitize_embate(embate: dict) -> dict:
        ...
        # Corrige datas
        try:
            data_inicio = datetime.fromisoformat(sanitized["data_inicio"].replace("Z", "+00:00"))
        ...
        if "argumentos" in sanitized:
            for arg in sanitized["argumentos"]:
                try:
                    data = datetime.fromisoformat(arg["data"].replace("Z", "+00:00"))
