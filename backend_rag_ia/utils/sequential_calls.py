...
class ChamadasSequenciaisManager:
    ...
    async def _criar_alerta(self, nivel: int) -> Dict:
        ...
        embate = Embate(
            ...
            metadata={"is_sequential_call_warning": True, "nivel": nivel, "severidade": severidade},
        )
        ...
        if nivel >= self.config.LIMITE_AVISO:
            sugestoes.extend(["Considere finalizar a sessão atual", "Revise as últimas alterações"])
