...
def calculate_document_hash(content: str, metadata: Dict[str, Any]) -> str:
    ...
    }, sort_keys=True, ensure_ascii=False)  # Preserva caracteres UTF-8
...
def check_document_exists(supabase: Client, doc_hash: str) -> bool:
    ...
    try:
        response = supabase.table("documents").select("id").eq("document_hash", doc_hash).execute()
...
def upload_document(supabase: Client, file_path: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if embedding_result.data:
            ...
            update_result = supabase.table("documents").update({"embedding_id": embedding_id}).eq("id", doc_id).execute()
    ...
    try:
        ...
        for title, ids in docs_by_title.items():
            if len(ids) > 1:
                ...
                console.print(f"  Removendo {len(ids_to_delete)} duplicatas de '{title}'")
