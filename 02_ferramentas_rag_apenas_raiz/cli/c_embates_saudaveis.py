#!/usr/bin/env python3
"""
Ferramenta CLI para gerenciamento de embates.
"""
...
from backend_rag_ia.cli.c_embates_saudaveis import Embate, Argumento, CondensadorEmbates
...
@click.command()
@click.option('--dir-embates', type=click.Path(exists=True), help='Diretório dos embates')
@click.option('--dir-regras', type=click.Path(exists=True), help='Diretório das regras')
@click.option('--min-embates', type=int, default=3, help='Mínimo de embates por tema')
def condensar(dir_embates: str, dir_regras: str, min_embates: int):
    """Condensa embates em regras."""
    condensador = CondensadorEmbates(
        dir_embates=dir_embates,
        dir_regras=dir_regras,
        min_embates_tema=min_embates
    )
    
    with Progress() as progress:
        task = progress.add_task("Condensando embates...", total=100)
        ...
        # Agrupa por tema
        grupos = condensador.agrupar_por_tema(embates)
        progress.update(task, advance=20)
        
        # Processa cada grupo
        for tema, embates_grupo in grupos.items():
            if len(embates_grupo) >= min_embates:
                # Gera regras
                conteudo = condensador.gerar_regras_md(tema, embates_grupo)
                progress.update(task, advance=20)
                ...
                # Arquiva embates processados
                condensador.arquivar_embates(embates_grupo)
                progress.update(task, advance=20)
                
                console.print(f"[green]Regras geradas para tema '{tema}' em {arquivo}[/green]")
