...
def verificar_pull_local():
    ...
    sucesso, resultado = executar_comando(
        "docker images fornalha/backend:latest --format '{{.ID}}'"
    )
    if sucesso and "1971c92916a2" in resultado:
        console.print(
            "[green]✅ Imagem correta já presente localmente (ID: 1971c92916a2)[/green]"
        )
        return True
    ...
    if sucesso:
        ...
        sucesso2, resultado2 = executar_comando(
            "docker images fornalha/backend:latest --format '{{.ID}}'"
        )
        if sucesso2 and "1971c92916a2" in resultado2:
            console.print(
                "[green]✅ Pull realizado com sucesso - Imagem correta (ID: 1971c92916a2)[/green]"
            )
            return True
        else:
            console.print(
                "[red]❌ Pull realizado mas ID da imagem não corresponde ao esperado[/red]"
            )
            return False
    else:
...
def verificar_variaveis():
    ...
    if sucesso:
        ...
        variaveis_necessarias = {"HOST": False, "PORT": False, "PYTHON_VERSION": False}
        ...
        faltando = [
            var for var, presente in variaveis_necessarias.items() if not presente
        ]
        if faltando:
...
def testar_local():
    ...
    if not sucesso or "1971c92916a2" not in resultado:
        console.print(
            "[red]❌ Imagem não encontrada localmente ou ID não corresponde[/red]"
        )
        return False
    ...
    sucesso, resultado = executar_comando(
        "docker run --rm fornalha/backend:latest python --version"
    )
    if not sucesso:
    ...
    sucesso, resultado = executar_comando(
        "docker images fornalha/backend:latest --format '{{.Size}}'"
    )
    if sucesso:
...
def testar_execucao_completa():
    ...
    for pacote in pacotes:
        ...
        comando = (
            f"docker run --rm fornalha/backend:latest python -c 'import {pacote}; print(\"OK\")'"
            ""
        )
