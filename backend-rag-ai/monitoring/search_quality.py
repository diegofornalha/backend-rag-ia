    ...
    def get_metricas(self) -> dict:
        ...
        if not self.buscas:
            return {
                ...
                "latencia": {"p50": 0.0, "p90": 0.0, "p99": 0.0, "min": 0.0, "max": 0.0},
            }
        ...
        feedback_positivo = len(
            [b for b in buscas_com_feedback if b.feedback.lower() in ["bom", "ótimo", "excelente"]]
        )

        taxa_feedback = feedback_positivo / len(buscas_com_feedback) if buscas_com_feedback else 0.0
    ...
    def get_queries_problematicas(self) -> list[str]:
        ...
        for busca in self.buscas:
            ...
            # Tempo alto
                ...
                problematicas.append(f"Tempo alto ({busca.tempo_resposta:.1f}s): {busca.query}")
