...
class LLMManager:
    ...
    async def process_query(self, query: str) -> dict[str, str]:
        ...
        try:
            ...
            context = {"query": query, "model": self.model, "tracker": self.llm_tracker}
            ...
            result = await self.coordinator.process(
                context=context, initial_analysis=cursor_analysis
            )

            return {"status": "success", "result": result, "agent_info": cursor_analysis.metadata}
